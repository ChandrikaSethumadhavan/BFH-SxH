...
class Model:
  ...
  def __init__(self, hp):
    self._hp              = hp
    self._train_flag      = tf.compat.v1.placeholder_with_default(
      tf.constant(True), shape=[], name="train_flag"
    )
    self._fetches         = {}
    self._fc_w = tf.compat.v1.get_variable(
      "fc_w",
      shape=[self._hp.n_cnn_outputs, self._hp.n_classes],
      dtype=tf.float32,
      initializer=tf.compat.v1.random_uniform_initializer(0, self._hp.ini_fc)
    )
    self._fc_b = tf.compat.v1.get_variable(
      "fc_b",
      shape=[1, self._hp.n_classes],
      dtype=tf.float32,
      initializer=tf.compat.v1.constant_initializer(gh.safe_get(self._hp, "ini_bias", 0.0))
    )
  ...
  def list_trainable_variables(self, listed_parts=None):
    model_parts = {
      ...
      "all"    : lambda: tf.compat.v1.trainable_variables()
    }
    if listed_parts:
      lst = []
      ...
      lst = tf.compat.v1.trainable_variables()
    return lst
